...
export default function SessionsPage() {
  ...
  return <WorkspaceShell admin={user.is_super_admin}><main className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-10">
    <header><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Account security</p><div className="mt-1 flex flex-wrap items-center justify-between gap-3"><h1 className="font-heading text-3xl font-bold">Active sessions</h1><div className="flex flex-wrap gap-4 text-sm"><Link href="/mfa" className="text-primary underline-offset-4 hover:underline">Two-factor authentication →</Link><Link href="/privacy-requests" className="text-primary underline-offset-4 hover:underline">Your data requests →</Link></div></div><p className="mt-2 text-sm leading-6 text-muted-foreground">Every device currently signed in to your account.</p></header>
