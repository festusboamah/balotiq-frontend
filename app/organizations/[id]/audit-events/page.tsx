...
export default function OrganizationAuditEventsPage() {
  ...
  return <WorkspaceShell admin={user.is_super_admin}><main className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-10">
    <header><Link href={`/organizations/${id}`} className="inline-flex min-h-11 items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" aria-hidden="true" />Organisation</Link><div className="mt-2 flex flex-wrap items-center justify-between gap-4"><h1 className="font-heading text-3xl font-bold">Audit trail</h1><Button type="button" variant="outline" onClick={async () => downloadBlob(await authGetBlob(`/api/v1/organizations/${id}/audit-events/export`), "audit-events.csv")}>Export CSV</Button></div></header>
