...
export default function PublicTicketingBuyPage() {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-2xl px-4 py-10 sm:px-6">
    <header>{event.cover_image_url && <img src={resolveSiteImageUrl(event.cover_image_url)} alt="" className="mb-6 h-72 w-full object-cover sm:h-96" />}<h1 className="font-heading text-3xl font-bold">{event.name}</h1>{event.venue && <p className="mt-1 text-sm text-muted-foreground">{event.venue}</p>}{event.event_starts_at && <p className="mt-1 text-sm text-muted-foreground">{new Date(event.event_starts_at).toLocaleString()}</p>}{event.description && <p className="mt-4 text-sm leading-6 text-muted-foreground">{event.description}</p>}</header>
