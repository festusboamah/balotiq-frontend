...
export default function EngageEventPage() {
  ...
  const uploadCoverImage = async (file: File) => {
    ...
    try { const body = new FormData(); body.append("file", file); await authPost(`/api/engage/events/${eventId}/cover-image`, body); refresh(); }
  ...
  return <WorkspaceShell admin={user.is_super_admin}><main className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-10">
    ...
    {canManage && nominations.length > 0 && <section aria-labelledby="nominations-heading"><h2 id="nominations-heading" className="font-heading text-2xl font-bold">Pending nominations</h2><div className="mt-4 space-y-3">{nominations.map(nomination => <div key={nomination.id} className="flex flex-wrap items-start justify-between gap-3 border bg-card p-4 text-sm"><div className="flex min-w-0 gap-3">{nomination.photo_url && <img src={resolveSiteImageUrl(nomination.photo_url)} alt="" className="h-14 w-14 shrink-0 rounded-full object-cover" />}<div className="min-w-0"><strong>{nomination.nominee_name}</strong>{nomination.nominee_reason && <p className="mt-1 text-xs text-muted-foreground">{nomination.nominee_reason}</p>}<p className="mt-1 text-xs text-muted-foreground">Submitted by {nomination.submitter_name} ({nomination.submitter_email})</p></div></div><div className="flex shrink-0 gap-2"><Button type="button" size="sm" onClick={() => void approveNomination(nomination.id)}>Approve</Button><Button type="button" size="sm" variant="destructive" onClick={() => void rejectNomination(nomination.id)}>Reject</Button></div></div>)}</div></section>}
    ...
    <section aria-labelledby="packages-heading"><h2 id="packages-heading" className="font-heading text-2xl font-bold">Vote packages</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{packages.length === 0 ? <p className="text-sm text-muted-foreground">No vote packages yet.</p> : packages.map(item => <div key={item.id} className="border bg-secondary/50 p-4 text-sm"><strong>{item.name}</strong><span className="ml-2 text-xs uppercase text-muted-foreground">{item.status}</span><span className="block text-xs text-muted-foreground">{item.currency} {item.amount} · {item.vote_quantity} vote(s)</span></div>)}</div>{canManage && hasConfiguration && <form onSubmit={addPackage} className="mt-4 grid gap-3 border-t pt-4 sm:grid-cols-3"><Input aria-label="Package name" required value={packageName} onChange={e => setPackageName(e.target.value)} placeholder="Package name" className="h-11" /><Input aria-label="Amount" required type="number" min="0" step="0.01" value={packageAmount} onChange={e => setPackageAmount(e.target.value)} placeholder="Amount (GHS)" className="h-11" /><Input aria-label="Vote quantity" required type="number" min="1" value={packageVotes} onChange={e => setPackageVotes(e.target.value)} placeholder="Votes" className="h-11" /><Button type="submit" disabled={pending} variant="outline" className="h-11 sm:col-span-3">Add package</Button></form>}</section>
