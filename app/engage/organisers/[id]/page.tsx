...
import type { EngageEventResponse, EngageOrganiserResponse } from "@/lib/types";
import { useRequireAuth } from "@/lib/use-require-auth";

export default function EngageOrganiserPage() {
  const { id } = useParams<{ id: string }>();
  const { user, loading, authGet, authPost, authPatch } = useRequireAuth();
  const [organiser, setOrganiser] = useState<EngageOrganiserResponse | null>(null);
  const [events, setEvents] = useState<EngageEventResponse[]>([]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      authGet<EngageOrganiserResponse>(`/api/engage/organisers/${id}`),
      authGet<EngageEventResponse[]>(`/api/engage/organisers/${id}/events`),
    ]).then(([record, rows]) => {
  ...
  const acceptAgreement = async () => {
    ...
    try { await authPost(`/api/engage/organisers/${id}/accept-agreement`, { accepted: true }); setReload(value => value + 1); }
  ...
  const saveFeeRate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { await authPatch(`/api/engage/organisers/${id}`, { platform_fee_rate: feeRate || null }); setReload(value => value + 1); }
  ...
  return <WorkspaceShell admin={user.is_super_admin}><main className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-10">
    ...
    <section aria-labelledby="events-heading"><div className="mb-4 flex flex-wrap items-center justify-between gap-4"><div><h2 id="events-heading" className="font-heading text-2xl font-bold">Events</h2><span className="text-sm text-muted-foreground">{events.length} total</span></div>{canManage && <Button render={<Link href={`/engage/organisers/${id}/events/new`} />} nativeButton={false}>New event</Button>}</div>{events.length === 0 ? <div className="border border-dashed bg-card p-8 text-center"><Sparkles className="mx-auto size-6 text-muted-foreground" aria-hidden="true" /><p className="mt-3 text-sm font-semibold">No events yet</p></div> : <div className="grid gap-3 sm:grid-cols-2">{events.map(item => <WorkspaceCard key={item.id} icon={Sparkles} href={`/engage/events/${item.id}`} title={item.name} detail={`/${item.slug}`} badge={item.status} />)}</div>}</section>
  </main></WorkspaceShell>;
}
